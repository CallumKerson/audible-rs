...
mod tests {
    ...
    async fn network_failure_is_not_fatal() {
        let (_dir, ctx) = test_ctx();
        // A loopback listener that accepts one connection and drops it at once
        // fails the request immediately, and — unlike releasing the port —
        // nothing else on the machine can bind it in the meantime.
